...
def plot_signal_background_histogram(y_test, y_scores):
    fig, ax = plt.subplots(figsize=(7, 7))
    plt.hist(y_scores[y_test == 1], bins=150, alpha=0.5, label='Dimuons (Signal)', color='blue')
    plt.hist(y_scores[y_test == 0], bins=150, alpha=0.5, label='Non-Dimuons (Background)', color='red')
...
def main():
    ...
    # Save model
    torch.save(trained_model, "/eos/user/e/ezaya/simulation_output/NN/NN_dimuons/Models/dimuon_selection_model.pt")
