...
bool filterDimuonsAfterMagnet(vector<double> &MAGNET_PDGID) {
  if (MAGNET_PDGID.size() != 2) return 0;

  int N_muons = 0;
  for (const auto& pdgId : MAGNET_PDGID) {
    if (pdgId == 13 || pdgId == -13) N_muons++;
  }

  if (N_muons != 2) return 0;

  return 1;
}

bool filterDimuAngles(double &DIMU_momX, double &DIMU_momZ){
  double angle = std::atan2(DIMU_momX, DIMU_momZ);
  return ((angle > -0.0225) && (angle < -0.0215));  
}

double SeperationDimuonsST(vector<double> &strawX, vector<int> &straw_PDGID) {
  int mpST = straw_PDGID.size();
  double mup_X = 0.;
  double mun_X = 0.;
  ...
  if(mpST > 4 && mpST < 9){
    for(int i = 0; i < mpST; ++i){
      if(straw_PDGID[i] == -13 && mup_found == false){
        mup_found = true;
        mup_X = strawX[i];  
      }
      if(straw_PDGID[i] == 13 && mun_found == false){
        mun_found = true;
        mun_X = strawX[i];
      }
      if (mup_found && mun_found) { break; } 
    }
  }
  return abs(mup_X - mun_X);
}

double SeperationMunST(vector<double> &strawX, vector<int> &straw_PDGID) {
  int mpST = straw_PDGID.size();
  double mun_X = 0.;
...
double TrackEnergyDimuonsST(vector<double> &Straw_TrackEnergy, vector<int> &straw_PDGID) {
  ...
  if(mpST > 4 && mpST < 9){
    for(int i = 0; i < mpST; ++i){
      if(straw_PDGID[i] == -13 && mup_found == false){
        mup_found = true;
        mup_E = Straw_TrackEnergy[i];
      }
      if(straw_PDGID[i] == 13 && mun_found == false){
        mun_found = true;
        mun_E = Straw_TrackEnergy[i];
      }
      if (mup_found && mun_found) { break; }
    }
  }
...
double TrackEnergyMupST(vector<double> &Straw_TrackEnergy, vector<int> &straw_PDGID) {
  int mpST = straw_PDGID.size();
  double mup_E = 0.;
...
double TrackEnergyMunST(vector<double> &Straw_TrackEnergy, vector<int> &straw_PDGID) {
  int mpST = straw_PDGID.size();
  double mun_E = 0.;
  bool mun_found = false;
...
double TrackEnergyDimuonsMM(vector<double> &MM_TrackEnergy, vector<int> &MM_PDGID) {
  int mpMM = MM_PDGID.size();
  double mup_E = 0.;
  double mun_E = 0.;
  bool mup_found = false;
  bool mun_found = false;
  ...
  if(mpMM > 1){
    for(int i = 0; i < mpMM; ++i){
      if(MM_PDGID[i] == -13 && mup_found == false){
        mup_found = true;
        mup_E = MM_TrackEnergy[i];
      }
      if(MM_PDGID[i] == 13 && mun_found == false){
        mun_found = true;
        mun_E = MM_TrackEnergy[i];
      }
      if (mup_found && mun_found) { break; }
    }
  }
...
double TrackEnergyMupMM(vector<double> &MM_TrackEnergy, vector<int> &MM_PDGID) {
  int mpMM = MM_PDGID.size();
  double mup_E = 0.;
...
double TrackEnergyMunMM(vector<double> &MM_TrackEnergy, vector<int> &MM_PDGID) {
  int mpMM = MM_PDGID.size();
  double mun_E = 0.;
...
std::vector<ROOT::RDF::RResultPtr<TH1>> doHisto(ROOT::RDF::RNode df, int icut, string cutName) {
  ...
  // STRAW 11 True information for 2024 analysis
  df = df.Define("TotalEnergyStraw11", TrackEnergyDimuonsST, { "Straw11_TrackEnergy", "Straw11_PDGID" });
  df = df.Define("TrackEnergyStraw11_mup", TrackEnergyMupST, { "Straw11_TrackEnergy", "Straw11_PDGID" });
  df = df.Define("TrackEnergyStraw11_mun", TrackEnergyMunST, { "Straw11_TrackEnergy", "Straw11_PDGID" });
  df = df.Define("SeperationDimuonsStraw11", SeperationDimuonsST, { "Strawx11_truth", "Straw11_PDGID" });
  df = df.Define("SeperationStraw11_mun", SeperationMunST, { "Strawx11_truth", "Straw11_PDGID" });
  df = df.Define("SeperationStraw11_mup", SeperationMupST, { "Strawx11_truth", "Straw11_PDGID" });

  // STRAW 12 True information for 2024 setup analysis
  df = df.Define("TotalEnergyStraw12", TrackEnergyDimuonsST, { "Straw12_TrackEnergy", "Straw12_PDGID" });
  df = df.Define("TrackEnergyStraw12_mup", TrackEnergyMupST, { "Straw12_TrackEnergy", "Straw12_PDGID" });
  df = df.Define("TrackEnergyStraw12_mun", TrackEnergyMunST, { "Straw12_TrackEnergy", "Straw12_PDGID" });
  df = df.Define("SeperationDimuonsStraw12", SeperationDimuonsST, { "Strawx12_truth", "Straw12_PDGID" });
  df = df.Define("SeperationStraw12_mun", SeperationMunST, { "Strawx12_truth", "Straw12_PDGID" });
  df = df.Define("SeperationStraw12_mup", SeperationMupST, { "Strawx12_truth", "Straw12_PDGID" });

  // MM 6 true information for 2024 setup analysis
  df = df.Define("TotalEnergyMM6", TrackEnergyDimuonsMM, { "MM6_E_truth", "MM6_PDGID" });
  df = df.Define("TrackEnergyMM6_mup", TrackEnergyMupMM, { "MM6_E_truth", "MM6_PDGID" });
  df = df.Define("TrackEnergyMM6_mun", TrackEnergyMunMM, { "MM6_E_truth", "MM6_PDGID" });
  
  // MM 7 true information for 2024 setup analysis
  df = df.Define("TotalEnergyMM7", TrackEnergyDimuonsMM, { "MM7_E_truth", "MM7_PDGID" });
  df = df.Define("TrackEnergyMM7_mup", TrackEnergyMupMM, { "MM7_E_truth", "MM7_PDGID" });
  df = df.Define("TrackEnergyMM7_mun", TrackEnergyMunMM, { "MM7_E_truth", "MM7_PDGID" });

  // Dimuon Seperation vs Track Energy
  ret.push_back(df.Histo2D( { Form("SeperationVSTotalEnergy_ST11_%i", icut), Form("%s ; Total Energy [GeV]; Seperation [mm]", cutName.c_str()), 60u, 0., 100., 60u, 0., 1200. }, "TotalEnergyStraw11", "SeperationDimuonsStraw11"));
  ret.push_back(df.Histo2D( { Form("SeperationVSTotalEnergy_ST12_%i", icut), Form("%s ; Total Energy [GeV]; Seperation [mm]", cutName.c_str()), 60u, 0., 100., 60u, 0., 1200. }, "TotalEnergyStraw12", "SeperationDimuonsStraw12"));
  ret.push_back(df.Histo2D( { Form("SeperationVSMupEnergy_ST11_%i", icut), Form("%s ; Mu_p Energy [GeV]; Seperation [mm]", cutName.c_str()), 60u, 1., 100., 60u, 0., 1200. }, "TrackEnergyStraw11_mup", "SeperationStraw11_mup"));
  ret.push_back(df.Histo2D( { Form("SeperationVSMunEnergy_ST11_%i", icut), Form("%s ; Mu_n Energy [GeV]; Seperation [mm]", cutName.c_str()), 60u, 1., 100., 60u, 0., 1200. }, "TrackEnergyStraw11_mun", "SeperationStraw11_mun")); 
  ret.push_back(df.Histo2D( { Form("SeperationVSMupEnergy_ST12_%i", icut), Form("%s ; Mu_p Energy [GeV]; Seperation [mm]", cutName.c_str()), 60u, 1., 100., 60u, 0., 1200. }, "TrackEnergyStraw12_mup", "SeperationStraw12_mup"));
  ret.push_back(df.Histo2D( { Form("SeperationVSMunEnergy_ST12_%i", icut), Form("%s ; Mu_n Energy [GeV]; Seperation [mm]", cutName.c_str()), 60u, 1., 100., 60u, 0., 1200. }, "TrackEnergyStraw12_mun", "SeperationStraw12_mun"));
  ...
  ret.push_back(df.Histo1D( { Form("Straw11_Energy_mun_%i", icut), "Straw11 mu_n Energy", 100u, 1., 100.}, "TrackEnergyStraw11_mun"));
  
  ret.push_back(df.Histo1D( { Form("Straw12_TotalEnergy_%i", icut), "Straw12 Total Energy", 100u, 0., 100.}, "TotalEnergyStraw12"));
  ret.push_back(df.Histo1D( { Form("Straw12_Energy_mup_%i", icut), "Straw12 mu_p Energy", 100u, 1., 100.}, "TrackEnergyStraw12_mup"));
  ret.push_back(df.Histo1D( { Form("Straw12_Energy_mun_%i", icut), "Straw12 mu_n Energy", 100u, 1., 100.}, "TrackEnergyStraw12_mun"));

  ret.push_back(df.Histo1D( { Form("MM6_TotalEnergy_%i", icut), "MM6 Total Energy", 100u, 0., 100.}, "TotalEnergyMM6"));
  ret.push_back(df.Histo1D( { Form("MM6_Energy_mup_%i", icut), "MM6 mu_p Energy", 100u, 1., 100.}, "TrackEnergyMM6_mup"));
  ret.push_back(df.Histo1D( { Form("MM6_Energy_mun_%i", icut), "MM6 mu_n Energy", 100u, 1., 100.}, "TrackEnergyMM6_mun"));

  ret.push_back(df.Histo1D( { Form("MM7_TotalEnergy_%i", icut), "MM7 Total Energy", 100u, 0., 100.}, "TotalEnergyMM7"));
  ret.push_back(df.Histo1D( { Form("MM7_Energy_mup_%i", icut), "MM7 mu_p Energy", 100u, 1., 100.}, "TrackEnergyMM7_mup"));
  ret.push_back(df.Histo1D( { Form("MM7_Energy_mun_%i", icut), "MM7 mu_n Energy", 100u, 1., 100.}, "TrackEnergyMM7_mun"));
...
void sim_ana_2024optSetup_mu(string fIn = "", string fOut = "") {
  ...
  auto dF14 = dMagnet.Filter(filterFunHCALtopo, { "eH_PDS" });
  ...
  auto dFAfterMagnet = dF19.Filter(filterDimuonsAfterMagnet, { "MAGNETPDGID" });
  postCutProcess(18, "After Magnet True Detector", dFAfterMagnet, 0);
  auto dF20 = dFAfterMagnet.Filter("DIMU_vertexZ > 2980.96");
  postCutProcess(19, "DIMU_BeforeECAL cut ", dF20, 0);
  auto dF21 = dF20.Filter("DIMU_vertexZ < 3430.96");
  postCutProcess(20, "DIMU_AfterECAL cut ", dF21, 0);
  auto dF22 = dF21.Filter(filterFunVETO, { "VETO" });
  postCutProcess(21, "VETO < 0.05", dF22, 0);
  auto dF23 = dF22.Filter(filterFunVETO2, { "VETO" });
  postCutProcess(22, "Veto23 > 0.01", dF23, 0);
  auto dF24 = dF23.Filter(filterMagnetIronBlock, { "Strawy11", "Strawx12", "Strawy12" });
  postCutProcess(23, "Magnet Acceptence (Iron Cut)", dF24, 0); 
  auto dF25 = dF24.Filter(filterLargeStrawMult, { "Strawx11", "Strawy11" });
  postCutProcess(24, "Straw Acceptence Cut", dF25, 0);
  ...
  postCutProcess(26, "Straw Single Muon Cut", dF27, 0);
  auto dF28 = dF24.Filter(filterSingleMuons, { "Straw12_PDGID" });
  postCutProcess(27, "Straw11 single muons cut [TRUE]", dF28, 0);
  auto dF29 = dF24.Filter(filterDimuons, { "Straw12_PDGID"});
  postCutProcess(28, "Straw11 dimuon seperation cut [TRUE]", dF29, 0);
