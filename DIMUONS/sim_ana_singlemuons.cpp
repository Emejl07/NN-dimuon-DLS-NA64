...
bool physTrigger(RVecD &PRS_ene, RVecD &ECAL_ene) {
  ...
  return ((sECAL_ene < 76)  && (sPS_ene > 0.4));
}
...
// Filter of straw11 w.r.t. position in x and y
bool filterStraw11(vector<double> &straw11X, vector<double> &straw11Y) {
  int leftSide = 0;
  int rightSide = 0;
...
std::vector<ROOT::RDF::RResultPtr<TH1>> doHisto(ROOT::RDF::RNode df, int icut, string cutName) {
  ...
  df = df.Define("eH0central", "eH_PDS[4]");// central cell HCAL0
  df = df.Define("eH1central", "eH_PDS[13]");// central cell HCAL1
  df = df.Define("eH2central", "eH_PDS[22]");// central cell HCAL2
  ...
  ret.push_back(df.Histo1D( { Form("Straw11X_%i", icut), "Straw11X", 200u, -1200., 1200.}, "Strawx11"));
  ret.push_back(df.Histo1D( { Form("Straw11Y_%i", icut), "Straw11Y", 200u, -600., 600.}, "Strawy11"));
  ret.push_back(df.Histo1D( { Form("Straw12X_%i", icut), "Straw12X", 200u, -1200., 1200.}, "Strawx12"));
  ret.push_back(df.Histo1D( { Form("Straw12Y_%i", icut), "Straw12Y", 200u, -600., 600.}, "Strawy12"));
  ret.push_back(df.Histo1D( { Form("Straw11X_true_%i", icut), "Straw11X true", 200u, -1200., 1200.}, "Strawx11_truth"));
  ...
  // MM
  ret.push_back(df.Histo1D( { Form("MM5X_%i", icut), "MM5X", 200u, -125.,125.}, "MM5x"));
  ret.push_back(df.Histo1D( { Form("MM5Y_%i", icut), "MM5Y", 200u, -45.,45.}, "MM5y"));
  ret.push_back(df.Histo1D( { Form("MM6X_%i", icut), "MM6X", 200u, -125.,125.}, "MM6x"));
  ret.push_back(df.Histo1D( { Form("MM6Y_%i", icut), "MM6Y", 200u, -45.,45.}, "MM6y"));
  ret.push_back(df.Histo1D( { Form("MM7X_%i", icut), "MM7X", 200u, -125.,125.}, "MM7x"));
  ret.push_back(df.Histo1D( { Form("MM7Y_%i", icut), "MM7Y", 200u, -45.,45.}, "MM7y"));
  ...
  ret.push_back(df.Histo1D( { Form("HCAL0central%i", icut), "HC0 central", 600u, 0., 120. }, "eH0central"));
  ret.push_back(df.Histo1D( { Form("HCAL1central%i", icut), "HC1 central", 600u, 0., 120. }, "eH1central"));
  ret.push_back(df.Histo1D( { Form("HCAL2central%i", icut), "HC2 central", 600u, 0., 120. }, "eH2central"));
...
void sim_ana_singlemuons(string fIn = "", string fOut = "") {
  ...
  //auto dBlind = d1.Filter("eH0_blind > 4 && eH1_blind > 4 && eH2_blind > 4");
  postCutProcess(0, "AllEvents",d1, 0);
  auto dF1 = d1.Filter(physTrigger, { "ECALpenergy", "ECALenergy" });
  ...
  auto dF24 = dF23.Filter(filterLargeStrawMult, { "Strawx11", "Strawy11" });
  postCutProcess(22, "Straw11 mult == 1 cut", dF24, 0);
  //auto dF25 = dF24.Filter(filterStraw11, { "Strawx11", "Strawy11" });
  //postCutProcess(23, "Straw11 cut", dF25, 0);
  //auto dF25 = dF24.Filter(filterLargeStrawMult, { "Strawx12", "Strawy12" });
  //postCutProcess(23, "Straw12 mult == 2 cut", dF25, 0);
...
// ------------------------------------------------------------------------------------------------------------ //
  ...
  postCutProcess(0, "AllEvents",d1, 1);
  auto dF1_ord = d1.Filter(physTrigger, { "ECALpenergy", "ECALenergy" });
  ...
  auto dF13_ord = dF12_ord.Filter(filterStraw11, { "Strawx11", "Strawy11" });
  ...
  auto dF15_ord = dF13_ord.Filter("Sum(Take(eH_PDS,-9))<2.");
  postCutProcess(14, "HC3", dF15_ord, 1);
  auto dF16_ord = dF15_ord.Filter(filterFunHCALtopo, { "eH_PDS" });
  postCutProcess(15, "HC0 topo", dF16_ord, 1);
  auto dF17_ord = dF16_ord.Filter(filterFunMuonHC2, { "eH_PDS" });
  postCutProcess(16, "HC2 muon cut", dF17_ord, 1);
  auto dF18_ord = dF17_ord.Filter(filterFunMuonHC1, { "eH_PDS" });
  postCutProcess(17, "HC1 muon cut", dF18_ord, 1);
  auto dF19_ord = dF18_ord.Filter(filterFunMuonHC0, { "eH_PDS" });
  postCutProcess(18, "HC0 muon cut", dF19_ord, 1);
  auto dF20_ord = dF19_ord.Filter("DIMU_NDimuonsTrue == 1");
  postCutProcess(19, "NDimuonsCut == 1", dF20_ord, 1);
  auto dF21_ord = dF20_ord.Filter("DIMU_vertexZ > 2980.96");
  postCutProcess(20, "DIMU_BeforeECAL cut ", dF21_ord, 1);
  auto dF22_ord = dF21_ord.Filter("DIMU_vertexZ < 3430.96");
  postCutProcess(21, "DIMU_AfterECAL cut ", dF22_ord, 1);
  auto dF23_ord = dF22_ord.Filter(filterFunVETO, { "VETO" });
  postCutProcess(22, "VETO < 0.05", dF23_ord, 1);
  auto dF24_ord = dF23_ord.Filter(filterFunVETO2, { "VETO" });
  postCutProcess(23, "Veto23 > 0.01", dF24_ord, 1);
  //auto dF24 = dF23.Filter(filterStraw11, { "Strawx11", "Strawx11" });
  //postCutProcess(22, "Straw11 mult == 2 cut", dF24);

  double bias = 15.;
  double EOT = 883834683 + 890922066;
  ...
  for (int ii = 1; ii < allN.size(); ii++) {
    double eff = (allN[ii - 1].GetValue() - allN[ii].GetValue()) / (EOT * bias);//(1. * allN[0].GetValue());
  ...
  cout << endl;
  cout << "Final eff: " << allN.back().GetValue() / (EOT * bias)  << std::endl;

  cout << endl;
  cout << "Ordered cutflow" << endl;
  printf("Cut:|CutName\t\t\t   |NumEvents\t\t|Eff in %%\t\t|EffRel in %%\t\t\n");
  for (int ii = 1; ii < allN_ord.size(); ii++) {
    double eff = (allN_ord[ii - 1].GetValue() - allN_ord[ii].GetValue()) / (EOT * bias);//(1. * allN_ord[0].GetValue());
