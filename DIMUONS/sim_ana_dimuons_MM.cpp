...
bool filterMMDimuonSelection(vector<double> &MM6x, vector<double> &MM7x, vector<int> &MM6_PDGID, vector<int> &MM7_PDGID) {
  if (MM6x.size() == 0 || MM7x.size() == 0) return 0;
  if((MM6_PDGID[0] == -13 && MM6x.size() == 1) && (MM7_PDGID[0] == 13 && MM7x.size() == 1)) return 1;
  return 0;
}

// Filter of straw11 so that we have only one hit in the + and - region.
bool filterMMBackgroundSelection(vector<double> &MM6x, vector<double> &MM7x, vector<int> &MM6_PDGID, vector<int> &MM7_PDGID) {
  if (MM6x.size() == 0 || MM7x.size() == 0) return 0;
  if(!((MM6x.size() == 1 && MM6_PDGID[0] != -13) && (MM7x.size() == 1 && MM7_PDGID[0] == 13))) return 1;
  return 0;
}
...
void sim_ana_dimuons_MM(string fIn = "", string fOut = "") {
  ...
  auto dF1 = dBlind.Filter(physTrigger, { "ECALpenergy", "ECALenergy" });
  ...
  // MM cut and analysis with true detecotors
  auto dF25 = dF24.Filter(filterMPMM, { "MM6_X_truth", "MM7_X_truth" });
  postCutProcess(23, "MM Multiplicity Cut [TRUE]", dF25, 0);
  auto dF26 = dF24.Filter(filterMMDimuonSelection, { "MM6_X_truth", "MM7_X_truth", "MM6_PDGID", "MM7_PDGID" });
  postCutProcess(24, "MM Dimuon Selection [TRUE]", dF26, 0);
  auto dF27 = dF24.Filter(filterMMBackgroundSelection, { "MM6_X_truth", "MM7_X_truth", "MM6_PDGID", "MM7_PDGID" });
  postCutProcess(25, "MM Background Selection [TRUE]", dF27, 0);
  auto dF28 = dF24.Filter(filterMPMM, { "MM6x", "MM7x" });
  postCutProcess(26, "MM Dimuon Cut", dF28, 0);

  auto dF29 = dF24.Filter(filterAcceptenceST, { "Straw11_PDGID", "Straw12_PDGID" });
  postCutProcess(27, "Straw Acceptence Cut [TRUE]", dF29, 0);
  auto dF30 = dF24.Filter(filterStrawFalseMuons, { "Straw11_PDGID" });
  postCutProcess(28, "Straw11 false muons cut [TRUE]", dF30, 0);
  auto dF31 = dF24.Filter(filterSingleMuons, { "Straw11_PDGID" });
  postCutProcess(29, "Straw11 single muons cut [TRUE]", dF31, 0);
  auto dF32 = dF24.Filter(filterStrawBackground, { "Straw11_PDGID" });
  postCutProcess(30, "Straw11 background cut [TRUE]", dF32, 0);
  auto dF33 = dF24.Filter(filterStraw11MuonSeperation, { "Strawx11_truth", "Straw11_PDGID"});
  postCutProcess(31, "Straw11 dimuon seperation cut [TRUE]", dF33, 0); 
  auto dF34 = dF24.Filter(filterLargeStrawMult, { "Strawx11", "Strawy11" });
  postCutProcess(32, "Straw Acceptence Cut", dF34, 0);
  auto dF35 = dF24.Filter(filterStraw11EachSide, { "Strawx11", "Strawy11" });
  postCutProcess(33, "Straw11 Dimuon cut", dF35, 0);
